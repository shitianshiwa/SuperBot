...
const pixivfanbox = async(context, header = "", proxy = { host: "", port: 0 }, ii = 0) => {
    ...
    let pic1 = temp.match(/(https|http):\/\/pixiv\.pximg\.net\/c\/1200x630_90_a2_g5\/fanbox\/public\/images\/post\/\d+\/cover\/.*?\.(jpeg|png|jpg)/g);
    let pic2 = temp.match(/(https|http):\/\/downloads\.fanbox\.cc\/images\/post\/\d+\/.*?\.(jpeg|png|jpg)/g);
